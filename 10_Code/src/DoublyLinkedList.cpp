...
DLX::DLX(std::vector<std::vector<int>>& matrix){
    ...
    header = new DLXNode();
    allocatedNodes.push_back(header);
    ++numColumns;
...
DLX::~DLX(){
    for(DLXNode* node : allocatedNodes){
        delete node;
    }
...
const std::vector<std::vector<int>>& DLX::getSolution() const{
    return inputMatrix;
}

bool DLX::hasSolution() const{
    return numValidSolutions > 0;
}

bool DLX::hasMultipleSolutions() const{
    return numValidSolutions > 1;
}
